...
example, a 16-bit LFSR can generate up to 65535 values before the sequence repeats. Each non-zero
state appears exactly once in the cycle. The all-zero state is invalid, since an LFSR initialized
to zero will remain stuck in that state.
